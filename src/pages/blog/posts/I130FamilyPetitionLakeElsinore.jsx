import React from 'react';
import BlogPostTemplate from '../BlogPostTemplate';

const I130FamilyPetitionLakeElsinore = ({ language }) => {
  const content = {
    en: {
      slug: 'i-130-family-petition-lake-elsinore',
      title: 'I-130 Family Petition Guide for Lake Elsinore Residents',
      subtitle: 'Complete Guide to Sponsoring Family Members for Green Cards',
      metaTitle: 'I-130 Family Petition Lake Elsinore | Sponsor Family Immigration Near Me',
      metaDescription: 'File I-130 family petitions in Lake Elsinore. U.S. citizens and green card holders can sponsor spouses, children, parents, and siblings for immigration to America.',
      keywords: [
        'I-130 petition Lake Elsinore',
        'family immigration sponsor',
        'green card family petition',
        'sponsor spouse immigration',
        'sponsor parents green card',
        'family reunification visa',
        'immigrant visa petition',
        'USCIS I-130 form',
        'family preference categories',
        'immigration lawyer Lake Elsinore'
      ],
      category: 'Family Immigration',
      location: 'Lake Elsinore',
      date: 'January 29, 2026',
      lastUpdated: 'January 29, 2026',
      readTime: '16 min read',
      author: 'SoCal Immigration Services',
      summary: 'The I-130 Petition for Alien Relative is the foundation of family-based immigration in America. Lake Elsinore residents who are U.S. citizens or permanent residents can sponsor qualifying family members for green cards. This guide explains eligibility, processing times, documentation requirements, and how to navigate the family petition process successfully.',
      locationIntro: 'Lake Elsinore\'s diverse community includes many families separated by borders. The city\'s 75,000+ residents include U.S. citizens and permanent residents eager to reunite with family members abroad. Understanding the I-130 petition process enables Lake Elsinore families to begin the legal pathway toward permanent family reunification.',
      sections: [
        {
          heading: 'What is Form I-130?',
          content: `Form I-130, Petition for Alien Relative, is the first step in most family-based green card applications. This form establishes the qualifying family relationship between the petitioner (U.S. citizen or permanent resident) and the beneficiary (foreign national family member).

**Key I-130 Facts:**
- **Filing Fee**: $675 (as of January 2026)
- **Processing Time**: 5-24 months depending on relationship
- **Filed With**: USCIS (U.S. Citizenship and Immigration Services)
- **Purpose**: Proves family relationship and petitioner's status

**Who Can File I-130:**

**U.S. Citizens Can Petition For:**
- Spouse (husband or wife)
- Unmarried children under 21 (immediate relatives)
- Unmarried children 21 and over
- Married children of any age
- Parents (petitioner must be 21+)
- Brothers and sisters (petitioner must be 21+)

**Permanent Residents (Green Card Holders) Can Petition For:**
- Spouse
- Unmarried children under 21
- Unmarried children 21 and over

**Important Limitations:**
- Grandparents cannot be directly petitioned
- Aunts, uncles, cousins not eligible for I-130
- In-laws cannot be petitioned
- Step-relationships require marriage before child turned 18
- Adopted children have specific requirements

Understanding these categories determines visa availability and waiting times for your Lake Elsinore family.`
        },
        {
          heading: 'Family Preference Categories and Wait Times',
          content: `Not all family relationships receive immediate visa availability. USCIS organizes family immigration into immediate relatives and preference categories with annual limits.

**Immediate Relatives (No Wait - U.S. Citizens Only):**
No visa number limits; processing is only limited by USCIS capacity:
- **IR-1**: Spouse of U.S. citizen
- **IR-2**: Unmarried child under 21 of U.S. citizen
- **IR-5**: Parent of U.S. citizen (citizen must be 21+)

**Family Preference Categories (Annual Limits Apply):**

**F1 - First Preference**
- Who: Unmarried adult children (21+) of U.S. citizens
- Annual Limit: 23,400 visas
- Current Wait: 7-8 years (varies by country)

**F2A - Second Preference A**
- Who: Spouses and unmarried children under 21 of green card holders
- Annual Limit: 87,934 visas (77% of F2)
- Current Wait: 2-3 years

**F2B - Second Preference B**
- Who: Unmarried adult children (21+) of green card holders
- Annual Limit: 26,266 visas (23% of F2)
- Current Wait: 6-9 years

**F3 - Third Preference**
- Who: Married children of U.S. citizens (any age)
- Annual Limit: 23,400 visas
- Current Wait: 12-15 years

**F4 - Fourth Preference**
- Who: Brothers and sisters of adult U.S. citizens
- Annual Limit: 65,000 visas
- Current Wait: 15-24 years

**Country-Specific Backlogs:**
Applicants from high-demand countries face longer waits:
- Mexico: Additional 5-10 years for most categories
- Philippines: Additional 5-15 years
- India: Additional 2-5 years
- China: Additional 2-5 years

Check the monthly Visa Bulletin for current wait times.`
        },
        {
          heading: 'I-130 Documentation Requirements',
          content: `Successful I-130 petitions require comprehensive documentation proving the family relationship and the petitioner's legal status.

**Petitioner Documentation:**
Evidence of U.S. citizenship or permanent residence:
- U.S. passport or birth certificate (citizens)
- Naturalization certificate (naturalized citizens)
- Green card (permanent residents)
- Consular Report of Birth Abroad

**Relationship Evidence by Category:**

**For Spouse Petitions:**
- Marriage certificate (foreign certificates need translation)
- Divorce decrees for any prior marriages (both parties)
- Death certificates if prior spouse deceased
- Evidence of bona fide marriage:
  - Joint bank accounts
  - Joint lease or mortgage
  - Joint insurance policies
  - Birth certificates of children together
  - Photos together over time
  - Affidavits from friends/family

**For Child Petitions:**
- Child's birth certificate showing parent's name
- Marriage certificate of parents (if born in wedlock)
- Adoption decree (if adopted)
- Evidence of custody and legal relationship
- For stepchildren: marriage certificate showing marriage occurred before child turned 18

**For Parent Petitions:**
- Petitioner's birth certificate showing parent's name
- Parent's birth certificate
- Marriage certificate of parents (if applicable)
- Evidence petitioner is 21 years or older

**For Sibling Petitions:**
- Both parties' birth certificates showing common parent
- Parent's marriage certificates (if different mothers)
- Petitioner's proof of U.S. citizenship
- Evidence petitioner is 21 years or older

**Document Tips:**
- All foreign documents need certified English translations
- Original documents or certified copies required
- Documents should be recent (within 6 months for some)
- Organize documents by category with clear labels`
        },
        {
          heading: 'I-130 Filing Process Step by Step',
          content: `Lake Elsinore residents can file I-130 petitions through USCIS online or by mail. Here's the complete process:

**Step 1: Determine Eligibility**
- Confirm you qualify as petitioner (citizen or LPR)
- Verify beneficiary is eligible family member
- Check preference category and estimated wait time
- Decide if I-130 alone or concurrent filing possible

**Step 2: Gather Documentation**
- Collect all required evidence
- Obtain certified translations of foreign documents
- Make copies of everything
- Organize documents clearly

**Step 3: Complete Form I-130**
- Download latest version from USCIS.gov
- Complete all sections accurately
- Sign and date the form
- Include supplement A if beneficiary is spouse

**Step 4: Pay Filing Fee**
- Current fee: $675
- Pay by check, money order, or credit card (online)
- Fee is per petition (one for each beneficiary)
- Some petitioners may qualify for fee waiver

**Step 5: Submit Petition**
- Online filing through USCIS account
- Mail to appropriate USCIS lockbox address
- Include G-1145 to receive electronic notifications
- Keep copies of everything submitted

**Step 6: Receive Receipt Notice**
- USCIS sends Form I-797C within 2-3 weeks
- Receipt number tracks your case
- Receipt shows priority date (important for waiting)

**Step 7: Respond to Requests for Evidence (if any)**
- USCIS may request additional documentation
- Response deadline typically 87 days
- Submit requested evidence promptly
- Failure to respond results in denial

**Step 8: Receive Decision**
- Approved: I-797 Notice of Action sent
- For immediate relatives: case moves to NVC
- For preference categories: wait for visa availability
- Denied: appeal rights explained in notice`
        },
        {
          heading: 'After I-130 Approval',
          content: `I-130 approval is just the first step. What happens next depends on your family member's location and the visa category.

**For Immediate Relatives (No Waiting Period):**

**If Beneficiary is Abroad:**
1. Case transfers to National Visa Center (NVC)
2. NVC assigns case number and invoice
3. Pay immigrant visa fee ($325)
4. Pay Affidavit of Support fee ($120)
5. Submit Form DS-260 (online visa application)
6. Submit civil documents to NVC
7. Complete Affidavit of Support (I-864)
8. NVC schedules visa interview at U.S. consulate
9. Attend interview, receive visa
10. Enter U.S. as permanent resident

**If Beneficiary is in U.S. (Eligible for Adjustment):**
1. File I-485 (Application to Register Permanent Residence)
2. File concurrently with I-130 or after approval
3. Include I-765 (work permit) and I-131 (travel permit)
4. Attend biometrics appointment
5. Attend adjustment interview
6. Receive green card if approved

**For Preference Categories (Waiting Period Required):**

1. Monitor Visa Bulletin monthly
2. Priority date is date USCIS received I-130
3. When priority date becomes "current," visa available
4. Case transfers to NVC or beneficiary files I-485
5. Process continues as above

**Maintaining Eligibility During Wait:**
- Petitioner must maintain status (citizen or LPR)
- If petitioner naturalizes, case may upgrade
- Beneficiary's circumstances affect category (marriage, age)
- Keep USCIS updated on address changes
- Preserve relationship (don't divorce if spouse petition)`
        },
        {
          heading: 'Common I-130 Issues and Solutions',
          content: `Many I-130 petitions face challenges that delay or prevent approval. Understanding common issues helps Lake Elsinore families avoid problems.

**Insufficient Relationship Evidence:**
- Problem: USCIS doubts bona fide relationship
- Solution: Submit comprehensive evidence packet
- Marriage cases: joint finances, photos, affidavits
- Parent/child cases: multiple documents showing relationship

**Prior Immigration Violations:**
- Problem: Beneficiary has unlawful presence or violations
- Bars may apply (3-year, 10-year, permanent)
- Solution: Consult attorney about waivers (I-601)
- Some categories allow waiver applications

**Criminal History:**
- Problem: Petitioner or beneficiary has criminal record
- Certain crimes make beneficiary inadmissible
- Solution: Review specific charges with attorney
- Some crimes have waivers available

**Marriage Fraud Concerns:**
- Problem: USCIS suspects marriage is fake
- Solution: Document relationship thoroughly
- Interviews test both parties separately
- Consistency and evidence are key

**Previous Petition Issues:**
- Problem: Petitioner filed multiple spouse petitions
- USCIS scrutinizes serial petitioners
- Solution: Explain prior marriages legitimately
- Provide evidence of genuine prior relationships

**Financial Insufficiency:**
- Problem: Petitioner can't meet income requirements
- Affidavit of Support requires 125% of poverty level
- Solution: Use joint sponsor if needed
- Include assets to supplement income

**Documentation Problems:**
- Problem: Missing or incorrect documents
- Solution: Respond completely to RFEs
- Obtain certified translations
- Request new documents if originals unavailable

**SoCal Immigration Services helps Lake Elsinore families:**
- Review cases before filing
- Prepare comprehensive petitions
- Respond to RFEs effectively
- Navigate complex situations`
        },
        {
          heading: 'Special I-130 Situations',
          content: `Certain family situations require additional steps or face unique rules in the I-130 process.

**Stepparent/Stepchild Relationships:**
- Marriage must occur before child turns 18
- Biological parent must be petitioning stepparent's spouse
- Same evidence requirements as biological relationships
- Birth certificate plus marriage certificate needed

**Adopted Children:**
- Adoption must be final and legal
- Child must be under 16 at adoption (exceptions exist)
- Two years legal custody and physical presence required
- IR-3 vs IR-4 depends on where adoption finalized

**Children Born Out of Wedlock:**
- Father petitioning: must legitimize child under local law
- Mother petitioning: standard birth certificate sufficient
- Evidence of father-child relationship helpful

**Widows and Widowers:**
- Can self-petition if married to U.S. citizen
- Must file within 2 years of spouse's death
- Marriage must have been bona fide
- No remarriage before approval

**Abused Family Members (VAWA):**
- Can self-petition without abuser's involvement
- Spouse, child, or parent of citizen/LPR abuser
- No need for abuser to file I-130
- Special protections and confidentiality

**Derivative Beneficiaries:**
- Spouse and unmarried children under 21 of principal
- Can immigrate with principal beneficiary
- Must maintain qualifying relationship
- Listed on principal's petition

**Petitioner's Death:**
- Some petitions can continue after death
- Humanitarian reinstatement possible
- Must have lived with petitioner as household member
- Consult attorney immediately if this occurs`
        },
        {
          heading: 'How SoCal Immigration Services Helps',
          content: `Navigating the I-130 process successfully requires attention to detail and understanding of immigration law. SoCal Immigration Services provides comprehensive support for Lake Elsinore families.

**Our I-130 Services:**

**Initial Consultation:**
- Evaluate family relationship and eligibility
- Determine appropriate category
- Explain wait times and process
- Identify potential issues early
- Develop case strategy

**Petition Preparation:**
- Complete I-130 form accurately
- Compile comprehensive evidence package
- Ensure all documents properly formatted
- Certified translations arranged
- Filing fee payment processed

**Case Management:**
- Track case status with USCIS
- Monitor receipt notices and updates
- Respond to Requests for Evidence
- Communicate with USCIS on your behalf
- Keep you informed throughout process

**Post-Approval Support:**
- Guide through NVC process
- Prepare for consular interview
- Assist with Affidavit of Support
- Help with adjustment of status if applicable
- Coordinate entire immigration journey

**Why Choose Us:**
- 20+ years family immigration experience
- Arabic, Spanish, and English services
- Personalized attention to each case
- Transparent pricing with no hidden fees
- Convenient service for Inland Empire residents

**Contact SoCal Immigration Services:**
Phone: (714) 909-7308
Languages: English, Arabic, Spanish
Service Area: Lake Elsinore and all Southern California

Start your family reunification journey with expert guidance from our experienced team.`
        }
      ],
      faqs: [
        {
          question: 'How long does the I-130 petition process take?',
          answer: 'I-130 processing time varies by relationship category and current USCIS workload. Immediate relative petitions (spouse, unmarried child under 21, parent of adult citizen) process in 5-12 months. Preference category petitions process in similar timeframes, but visa availability requires additional waiting: F2A (spouse/child of green card holder) waits 2-3 years; F1 (unmarried adult child of citizen) waits 7-8 years; F2B (unmarried adult child of LPR) waits 6-9 years; F3 (married child of citizen) waits 12-15 years; F4 (sibling of citizen) waits 15-24 years. Country-specific backlogs add additional time for applicants from Mexico, Philippines, India, and China.'
        },
        {
          question: 'Can I sponsor my parents for green cards?',
          answer: 'Yes, U.S. citizens who are 21 years or older can sponsor their parents for green cards. Parents qualify as immediate relatives, meaning no visa number limits or waiting periods apply. After filing Form I-130, processing typically takes 5-12 months before the case moves to the National Visa Center. Both mother and father require separate I-130 petitions. Green card holders cannot petition for parents; you must naturalize first. Stepparents can be petitioned if the marriage creating the stepparent relationship occurred before you turned 18.'
        },
        {
          question: 'What happens if my I-130 petition is denied?',
          answer: 'If USCIS denies your I-130, the denial notice explains the reason and appeal options. Common denial reasons include insufficient evidence of relationship, ineligible family relationship, or petitioner status issues. You can file Form I-290B (Notice of Appeal) within 30 days to the Administrative Appeals Office (AAO). Alternatively, you can file a motion to reopen (with new evidence) or motion to reconsider (arguing USCIS made legal error). Many denials result from fixable documentation issues. Consulting an immigration attorney helps evaluate whether to appeal, file motions, or submit a new petition with stronger evidence.'
        },
        {
          question: 'Can a green card holder sponsor a spouse?',
          answer: 'Yes, green card holders (permanent residents) can sponsor spouses for green cards through Form I-130. Spouses of green card holders fall into the F2A preference category, which has annual visa limits. Current wait times for F2A range from 2-3 years depending on the beneficiary\'s country of birth. During the waiting period, the spouse may remain abroad or, if legally present in the U.S., may be able to adjust status when a visa becomes available. If the green card holder naturalizes to become a U.S. citizen while the case is pending, the spouse upgrades to immediate relative status with no waiting period.'
        },
        {
          question: 'Do I need a lawyer to file I-130?',
          answer: 'While legal representation is not legally required for I-130 petitions, working with an experienced immigration attorney significantly improves outcomes. Attorneys help ensure forms are completed correctly, compile comprehensive evidence packages, identify potential issues before filing, respond effectively to Requests for Evidence, and navigate complex situations like prior immigration violations or criminal history. For straightforward immediate relative cases, some people successfully file themselves. However, any complications in family history, documentation, or immigration background warrant professional guidance. SoCal Immigration Services offers consultations to assess your specific situation.'
        }
      ],
      cta: {
        title: 'Ready to Sponsor Your Family Member?',
        text: 'SoCal Immigration Services helps Lake Elsinore families navigate the I-130 petition process with expert guidance and personalized support. Contact us to start reuniting your family through proper legal channels.',
        button: 'Schedule a Consultation',
        phone: '(714) 909-7308'
      }
    },
    ar: {
      slug: 'i-130-family-petition-lake-elsinore',
      title: 'دليل عريضة العائلة I-130 لسكان ليك إلسينور',
      subtitle: 'دليل شامل لكفالة أفراد العائلة للحصول على البطاقات الخضراء',
      metaTitle: 'عريضة العائلة I-130 ليك إلسينور | كفالة هجرة العائلة بالقرب مني',
      metaDescription: 'قدم عرائض العائلة I-130 في ليك إلسينور. يمكن للمواطنين الأمريكيين وحاملي البطاقات الخضراء كفالة الأزواج والأطفال والآباء والأشقاء للهجرة إلى أمريكا.',
      keywords: [
        'عريضة I-130 ليك إلسينور',
        'كفيل هجرة العائلة',
        'عريضة البطاقة الخضراء العائلية',
        'كفالة هجرة الزوج',
        'كفالة البطاقة الخضراء للوالدين',
        'تأشيرة لم شمل العائلة',
        'عريضة تأشيرة المهاجر',
        'نموذج USCIS I-130',
        'فئات تفضيل العائلة',
        'محامي هجرة ليك إلسينور'
      ],
      category: 'هجرة العائلة',
      location: 'ليك إلسينور',
      date: '٢٩ يناير ٢٠٢٦',
      lastUpdated: '٢٩ يناير ٢٠٢٦',
      readTime: '١٦ دقيقة قراءة',
      author: 'خدمات الهجرة في جنوب كاليفورنيا',
      summary: 'عريضة I-130 للقريب الأجنبي هي أساس الهجرة القائمة على العائلة في أمريكا. يمكن لسكان ليك إلسينور من المواطنين الأمريكيين أو المقيمين الدائمين كفالة أفراد العائلة المؤهلين للحصول على البطاقات الخضراء. يشرح هذا الدليل الأهلية وأوقات المعالجة ومتطلبات التوثيق.',
      locationIntro: 'يشمل مجتمع ليك إلسينور المتنوع العديد من العائلات المفصولة بالحدود. يشمل سكان المدينة البالغ عددهم أكثر من 75,000 مواطنين أمريكيين ومقيمين دائمين حريصين على لم شمل أفراد العائلة في الخارج.',
      sections: [
        {
          heading: 'ما هو نموذج I-130؟',
          content: `نموذج I-130، عريضة للقريب الأجنبي، هو الخطوة الأولى في معظم طلبات البطاقة الخضراء القائمة على العائلة. يثبت هذا النموذج العلاقة العائلية المؤهلة بين مقدم العريضة والمستفيد.

**حقائق I-130 الرئيسية:**
- **رسوم التقديم**: 675 دولار
- **وقت المعالجة**: 5-24 شهراً حسب العلاقة
- **يُقدم إلى**: USCIS
- **الغرض**: إثبات العلاقة العائلية ووضع مقدم العريضة

**من يمكنه تقديم I-130:**

**المواطنون الأمريكيون يمكنهم تقديم عريضة لـ:**
- الزوج/الزوجة
- الأطفال غير المتزوجين دون 21 سنة
- الأطفال غير المتزوجين 21 سنة فأكثر
- الأطفال المتزوجين من أي عمر
- الوالدين (يجب أن يكون مقدم العريضة 21+)
- الإخوة والأخوات (يجب أن يكون مقدم العريضة 21+)

**المقيمون الدائمون يمكنهم تقديم عريضة لـ:**
- الزوج/الزوجة
- الأطفال غير المتزوجين دون 21 سنة
- الأطفال غير المتزوجين 21 سنة فأكثر`
        },
        {
          heading: 'فئات تفضيل العائلة وأوقات الانتظار',
          content: `ليس كل العلاقات العائلية تتلقى توفر تأشيرة فوري. تنظم USCIS هجرة العائلة إلى أقارب مباشرين وفئات تفضيل بحدود سنوية.

**الأقارب المباشرون (بدون انتظار - للمواطنين الأمريكيين فقط):**
- IR-1: زوج/زوجة المواطن الأمريكي
- IR-2: طفل غير متزوج دون 21 من مواطن أمريكي
- IR-5: والد المواطن الأمريكي (المواطن يجب أن يكون 21+)

**فئات التفضيل العائلي:**

**F1 - التفضيل الأول**
- من: الأطفال البالغون غير المتزوجين (21+) للمواطنين
- الانتظار الحالي: 7-8 سنوات

**F2A - التفضيل الثاني أ**
- من: الأزواج والأطفال غير المتزوجين دون 21 لحاملي البطاقات الخضراء
- الانتظار الحالي: 2-3 سنوات

**F2B - التفضيل الثاني ب**
- من: الأطفال البالغون غير المتزوجين (21+) لحاملي البطاقات الخضراء
- الانتظار الحالي: 6-9 سنوات

**F3 - التفضيل الثالث**
- من: الأطفال المتزوجون للمواطنين الأمريكيين
- الانتظار الحالي: 12-15 سنة

**F4 - التفضيل الرابع**
- من: الإخوة والأخوات للمواطنين الأمريكيين البالغين
- الانتظار الحالي: 15-24 سنة`
        },
        {
          heading: 'متطلبات توثيق I-130',
          content: `تتطلب عرائض I-130 الناجحة توثيقاً شاملاً يثبت العلاقة العائلية والوضع القانوني لمقدم العريضة.

**توثيق مقدم العريضة:**
- جواز السفر الأمريكي أو شهادة الميلاد (للمواطنين)
- شهادة التجنيس (للمواطنين المتجنسين)
- البطاقة الخضراء (للمقيمين الدائمين)

**دليل العلاقة حسب الفئة:**

**لعرائض الزوج/الزوجة:**
- شهادة الزواج
- قرارات الطلاق لأي زيجات سابقة
- دليل الزواج الحقيقي:
  - حسابات بنكية مشتركة
  - عقد إيجار أو رهن عقاري مشترك
  - وثائق تأمين مشتركة
  - شهادات ميلاد الأطفال معاً
  - صور معاً عبر الزمن

**لعرائض الأطفال:**
- شهادة ميلاد الطفل تظهر اسم الوالد
- شهادة زواج الوالدين
- قرار التبني (إذا كان متبنى)

**لعرائض الوالدين:**
- شهادة ميلاد مقدم العريضة تظهر اسم الوالد
- شهادة ميلاد الوالد
- دليل أن مقدم العريضة 21 سنة أو أكثر`
        },
        {
          heading: 'عملية تقديم I-130 خطوة بخطوة',
          content: `يمكن لسكان ليك إلسينور تقديم عرائض I-130 عبر الإنترنت أو بالبريد.

**الخطوة 1: تحديد الأهلية**
- تأكد أنك مؤهل كمقدم عريضة
- تحقق أن المستفيد فرد عائلة مؤهل
- راجع فئة التفضيل ووقت الانتظار المقدر

**الخطوة 2: جمع التوثيق**
- اجمع كل الأدلة المطلوبة
- احصل على ترجمات معتمدة للوثائق الأجنبية
- اصنع نسخاً من كل شيء

**الخطوة 3: إكمال نموذج I-130**
- حمّل أحدث نسخة من USCIS.gov
- أكمل جميع الأقسام بدقة
- وقّع وأرّخ النموذج

**الخطوة 4: دفع رسوم التقديم**
- الرسوم الحالية: 675 دولار
- ادفع بشيك أو حوالة مالية أو بطاقة ائتمان

**الخطوة 5: تقديم العريضة**
- التقديم عبر الإنترنت من خلال حساب USCIS
- أو بالبريد إلى عنوان صندوق USCIS المناسب

**الخطوة 6: استلام إشعار الاستلام**
- USCIS ترسل النموذج I-797C خلال 2-3 أسابيع
- رقم الاستلام يتتبع قضيتك

**الخطوة 7: الرد على طلبات الأدلة**
- قد تطلب USCIS وثائق إضافية
- قدم الأدلة المطلوبة بسرعة

**الخطوة 8: استلام القرار**
- الموافقة: يُرسل إشعار I-797
- الرفض: حقوق الاستئناف موضحة في الإشعار`
        },
        {
          heading: 'بعد موافقة I-130',
          content: `موافقة I-130 هي الخطوة الأولى فقط. ما يحدث بعد ذلك يعتمد على موقع فرد عائلتك وفئة التأشيرة.

**للأقارب المباشرين (بدون فترة انتظار):**

**إذا كان المستفيد في الخارج:**
1. تُنقل القضية إلى مركز التأشيرات الوطني (NVC)
2. NVC يعين رقم قضية وفاتورة
3. ادفع رسوم تأشيرة المهاجر (325 دولار)
4. قدم النموذج DS-260 عبر الإنترنت
5. قدم الوثائق المدنية إلى NVC
6. أكمل إفادة الدعم (I-864)
7. NVC يحدد موعد مقابلة التأشيرة
8. احضر المقابلة، استلم التأشيرة
9. ادخل الولايات المتحدة كمقيم دائم

**إذا كان المستفيد في الولايات المتحدة:**
1. قدم I-485 (طلب تسجيل الإقامة الدائمة)
2. قدم متزامناً مع I-130 أو بعد الموافقة
3. أرفق I-765 (تصريح عمل) و I-131 (تصريح سفر)
4. احضر موعد القياسات الحيوية
5. احضر مقابلة التعديل
6. استلم البطاقة الخضراء إذا وافقوا

**لفئات التفضيل (فترة انتظار مطلوبة):**
1. راقب نشرة التأشيرات شهرياً
2. تاريخ الأولوية هو تاريخ استلام USCIS لـ I-130
3. عندما يصبح تاريخ الأولوية "حالياً"، التأشيرة متاحة`
        },
        {
          heading: 'مشاكل I-130 الشائعة والحلول',
          content: `تواجه كثير من عرائض I-130 تحديات تؤخر أو تمنع الموافقة.

**دليل علاقة غير كافٍ:**
- المشكلة: USCIS تشك في العلاقة الحقيقية
- الحل: قدم حزمة أدلة شاملة
- قضايا الزواج: مالية مشتركة، صور، إفادات

**انتهاكات هجرة سابقة:**
- المشكلة: المستفيد لديه إقامة غير قانونية
- قد تُطبق عقوبات الحظر (3 سنوات، 10 سنوات، دائم)
- الحل: استشر محامياً حول الإعفاءات (I-601)

**تاريخ جنائي:**
- المشكلة: مقدم العريضة أو المستفيد لديه سجل جنائي
- جرائم معينة تجعل المستفيد غير مقبول
- الحل: راجع التهم المحددة مع محامٍ

**مخاوف احتيال الزواج:**
- المشكلة: USCIS تشك أن الزواج مزيف
- الحل: وثّق العلاقة بدقة
- المقابلات تختبر الطرفين بشكل منفصل

**عدم كفاية مالية:**
- المشكلة: مقدم العريضة لا يستطيف تلبية متطلبات الدخل
- الحل: استخدم كفيل مشترك إذا لزم الأمر
- أرفق الأصول لتكملة الدخل`
        },
        {
          heading: 'حالات I-130 الخاصة',
          content: `بعض الحالات العائلية تتطلب خطوات إضافية أو تواجه قواعد فريدة.

**علاقات زوج/زوجة الأب/الأم والأطفال:**
- يجب أن يحدث الزواج قبل أن يبلغ الطفل 18
- نفس متطلبات الأدلة كالعلاقات البيولوجية

**الأطفال المتبنون:**
- يجب أن يكون التبني نهائياً وقانونياً
- يجب أن يكون الطفل دون 16 عند التبني
- مطلوب سنتان حضانة قانونية وإقامة فعلية

**الأطفال المولودون خارج الزواج:**
- الأب يقدم: يجب تشريع الطفل بموجب القانون المحلي
- الأم تقدم: شهادة الميلاد القياسية كافية

**الأرامل والأرملات:**
- يمكن تقديم عريضة ذاتية إذا كانوا متزوجين من مواطن أمريكي
- يجب التقديم خلال سنتين من وفاة الزوج
- يجب أن يكون الزواج حقيقياً
- عدم الزواج مرة أخرى قبل الموافقة

**أفراد العائلة المعنفون (VAWA):**
- يمكن تقديم عريضة ذاتية بدون تورط المعنف
- الزوج/الزوجة، الطفل، أو والد المواطن/LPR المعنف
- لا حاجة لأن يقدم المعنف I-130`
        },
        {
          heading: 'كيف تساعد خدمات الهجرة في جنوب كاليفورنيا',
          content: `يتطلب التنقل في عملية I-130 بنجاح اهتماماً بالتفاصيل وفهماً لقانون الهجرة. توفر خدمات الهجرة في جنوب كاليفورنيا دعماً شاملاً لعائلات ليك إلسينور.

**خدمات I-130 لدينا:**

**الاستشارة الأولية:**
- تقييم العلاقة العائلية والأهلية
- تحديد الفئة المناسبة
- شرح أوقات الانتظار والعملية
- تحديد المشاكل المحتملة مبكراً

**إعداد العريضة:**
- إكمال نموذج I-130 بدقة
- تجميع حزمة أدلة شاملة
- ضمان تنسيق جميع الوثائق بشكل صحيح
- ترتيب الترجمات المعتمدة

**إدارة القضية:**
- تتبع حالة القضية مع USCIS
- مراقبة إشعارات الاستلام والتحديثات
- الرد على طلبات الأدلة
- التواصل مع USCIS نيابة عنك

**اتصل بخدمات الهجرة في جنوب كاليفورنيا:**
الهاتف: (714) 909-7308
اللغات: الإنجليزية، العربية، الإسبانية

ابدأ رحلة لم شمل عائلتك مع توجيه خبير من فريقنا ذو الخبرة.`
        }
      ],
      faqs: [
        {
          question: 'كم يستغرق وقت عملية عريضة I-130؟',
          answer: 'يختلف وقت معالجة I-130 حسب فئة العلاقة وعبء العمل الحالي لـ USCIS. عرائض الأقارب المباشرين تُعالج في 5-12 شهراً. فئات التفضيل تتطلب انتظاراً إضافياً: F2A ينتظر 2-3 سنوات؛ F1 ينتظر 7-8 سنوات؛ F2B ينتظر 6-9 سنوات؛ F3 ينتظر 12-15 سنة؛ F4 ينتظر 15-24 سنة.'
        },
        {
          question: 'هل يمكنني كفالة والديّ للحصول على البطاقات الخضراء؟',
          answer: 'نعم، المواطنون الأمريكيون الذين يبلغون 21 سنة أو أكثر يمكنهم كفالة والديهم للحصول على البطاقات الخضراء. الوالدان مؤهلون كأقارب مباشرين، مما يعني عدم وجود حدود لأرقام التأشيرات أو فترات انتظار. يتطلب كل من الأم والأب عرائض I-130 منفصلة. حاملو البطاقات الخضراء لا يمكنهم تقديم عرائض للوالدين؛ يجب التجنس أولاً.'
        },
        {
          question: 'ماذا يحدث إذا رُفضت عريضة I-130 الخاصة بي؟',
          answer: 'إذا رفضت USCIS عريضة I-130 الخاصة بك، يشرح إشعار الرفض السبب وخيارات الاستئناف. يمكنك تقديم النموذج I-290B خلال 30 يوماً. بدلاً من ذلك، يمكنك تقديم طلب إعادة فتح (بأدلة جديدة) أو طلب إعادة النظر. كثير من حالات الرفض تنتج عن مشاكل توثيق قابلة للإصلاح.'
        },
        {
          question: 'هل يمكن لحامل البطاقة الخضراء كفالة زوج/زوجة؟',
          answer: 'نعم، حاملو البطاقات الخضراء يمكنهم كفالة الأزواج للحصول على البطاقات الخضراء من خلال نموذج I-130. أزواج حاملي البطاقات الخضراء يقعون في فئة التفضيل F2A، التي لها حدود تأشيرات سنوية. أوقات الانتظار الحالية لـ F2A تتراوح من 2-3 سنوات. إذا تجنس حامل البطاقة الخضراء ليصبح مواطناً أمريكياً أثناء انتظار القضية، يترقى الزوج/الزوجة إلى وضع قريب مباشر بدون فترة انتظار.'
        },
        {
          question: 'هل أحتاج محامياً لتقديم I-130؟',
          answer: 'بينما التمثيل القانوني غير مطلوب قانونياً لعرائض I-130، العمل مع محامي هجرة ذو خبرة يحسن النتائج بشكل كبير. المحامون يساعدون في ضمان إكمال النماذج بشكل صحيح، تجميع حزم أدلة شاملة، تحديد المشاكل المحتملة، والرد بفعالية على طلبات الأدلة. أي تعقيدات في تاريخ العائلة أو التوثيق أو خلفية الهجرة تستوجب توجيهاً مهنياً.'
        }
      ],
      cta: {
        title: 'هل أنت مستعد لكفالة فرد عائلتك؟',
        text: 'تساعد خدمات الهجرة في جنوب كاليفورنيا عائلات ليك إلسينور في التنقل في عملية عريضة I-130 بتوجيه خبير ودعم شخصي. اتصل بنا لبدء لم شمل عائلتك من خلال القنوات القانونية الصحيحة.',
        button: 'حدد موعد استشارة',
        phone: '(714) 909-7308'
      }
    }
  };

  return <BlogPostTemplate content={content} language={language} />;
};

export default I130FamilyPetitionLakeElsinore;
